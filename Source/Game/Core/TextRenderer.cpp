...
void TextRenderer::DrawChar(char16_t C, const FVector2& Position, float Scale)
{
	auto  Source = GetCharRect(C);
	FRect Dest{ Position, GGlyphRenderSize * Scale };
	GRenderer->DrawTexture(GFontTexture, Source, Dest);
}
void TextRenderer::DrawText(const std::string& Text, const FVector2& Position, float Scale)
{
	std::u16string Text16 = Converter.from_bytes(Text);
	DrawText(Text16, Position, Scale);
}

void TextRenderer::DrawText(const std::u16string& Text, const FVector2& Position, float Scale)
{
	int XOffset = 0;
	int YOffset = 0;
	...
	for (auto C : Text)
	{
		...
		FVector2 Offset = GGlyphRenderSize * Scale * FVector2(XOffset, YOffset);
		DrawChar(C, Position + Offset, Scale);
		XOffset++;
	}
}